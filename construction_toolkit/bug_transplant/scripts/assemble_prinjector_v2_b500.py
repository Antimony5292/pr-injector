...
def read_injection_index(run_dirs: list[Path]) -> dict[str, dict[str, Any]]:
    ...
    for run_dir in run_dirs:
        # Direct-agent rescue and environment-retry lanes do not necessarily
        # use the historical shard_new_l1l2_* directory naming convention.
        # The result filename is the stable construction contract.
        for path in sorted(run_dir.rglob("verified_injection_results.jsonl")):
            for row in read_jsonl(path):
                iid = row_id(row)
                if iid:
                    copied = dict(row)
                    ...
                    if existing is None or injection_preference(copied) > injection_preference(existing):
                        index[iid] = copied
    return index


def injection_preference(row: dict[str, Any]) -> tuple[int, int, float, str]:
    """Prefer a verified generation attempt over stale failures for the same ID."""
    gate = row.get("v2_fidelity_gate") or {}
    return (
        int(row.get("success") is True),
        int(row.get("v2_fidelity_gate_pass") is True or gate.get("pass_gate") is True),
        float(gate.get("score") or 0.0),
        str(row.get("construction_injection_source") or ""),
    )
...
def verification_rows(run_dirs: list[Path]) -> list[dict[str, Any]]:
    ...
    for run_dir in run_dirs:
        for path in sorted(run_dir.rglob("verified_verification_results.jsonl")):
            for row in read_jsonl(path):
                copied = dict(row)
...
def strict_and_v2_ok(
    ...
    target_retention = verification.get("target_retention_ratio")
    if target_retention is not None and float(target_retention) < 0.50:
        return False, "target_surface_collapse", {
            "target_retention_ratio": float(target_retention),
            "verified_target_count": int(verification.get("verified_target_count") or 0),
            "collectable_target_count_before_healthy_filter": int(
                verification.get("collectable_target_count_before_healthy_filter") or 0
            ),
        }
    ...
    construction_gate = injection.get("v2_fidelity_gate") or {}
    # Final verification intentionally executes a bounded clean P2P sample.
    # Comparing that sample (often 26-32 tests) with the full A-side surface
    # (sometimes >1,000 tests in SWE-bench-Live) creates a false fidelity
    # failure. Use the full construction-time mapped counts for the fidelity
    # ratio, while the strict checks above remain responsible for proving that
    # every sampled P2P test is safe in buggy and repaired states.
    a_p2p_surface = construction_gate.get("A_PASS_TO_PASS_count")
    if a_p2p_surface is None:
        a_p2p_surface = candidate.get("pass_to_pass") or candidate.get("PASS_TO_PASS") or []
    b_p2p_surface = construction_gate.get("B_PASS_TO_PASS_count")
    if b_p2p_surface is None:
        b_p2p_surface = clean_p2p
    gate = evaluate_patch_pair_fidelity(
        ...
        a_pass_to_pass=a_p2p_surface,
        b_pass_to_pass=b_p2p_surface,
...
def select_final(
    ...
    # A frozen source family can legitimately remain below its historical
    # quota (for example, no more original SWE-bench tasks are admitted). Do
    # not let that make a 500-case assembly mathematically impossible. Fill
    # the residual with strict rows from the admitted families, preferring the
    # most underfilled quota ratio and still enforcing the repo cap.
    target_size = sum(quotas.values())
    while len(selected) < target_size:
        spill_candidates: list[dict[str, Any]] = []
        for dataset, candidates in by_dataset.items():
            for row in candidates:
                iid = row_id(row)
                repo = str(row.get("repo") or "")
                if iid and iid not in selected_ids and repo_counts[repo] < repo_cap:
                    spill_candidates.append(row)
        if not spill_candidates:
            break
        spill_candidates.sort(
            key=lambda row: (
                dataset_counts[str(row.get("source_dataset") or "")]
                / max(quotas.get(str(row.get("source_dataset") or ""), 1), 1),
                0 if "Pro" in str(row.get("source_dataset") or "") else 1,
                row_rank(row, repo_counts, dataset_counts),
            )
        )
        picked = spill_candidates[0]
        iid = row_id(picked)
        dataset = str(picked.get("source_dataset") or "")
        repo = str(picked.get("repo") or "")
        picked["v2_quota_spill_fill"] = {
            "reason": "fill_frozen_source_quota_shortfall",
            "dataset_count_before": dataset_counts[dataset],
            "dataset_quota": quotas.get(dataset),
        }
        selected.append(picked)
        selected_ids.add(iid)
        dataset_counts[dataset] += 1
