...
RUN_TAG="${PRI_V2_RUN_TAG:-20260625}"
OUT_DIR="experiments/rq2_500/v2_construction_${RUN_TAG}"
CANDIDATES="experiments/rq2_500/v2_construction_pool_20260625/v2_main_construction_candidates.jsonl"
LOG_DIR="experiments/rq2_500/logs"
LOG_FILE="${LOG_DIR}/prinjector_v2_b500_construction_${RUN_TAG}.log"

mkdir -p "${OUT_DIR}" "${LOG_DIR}"
exec >>"${LOG_FILE}" 2>&1

echo
echo "----- start PR-INJECTOR v2 B500 construction ${RUN_TAG} -----"
date
echo "root=${ROOT}"
...
"${PY}" scripts/run_rq2_b500_fidelity_new_l1l2_shards_20260613.py \
  ...
  --ignore-processed \
  --limit 450 \
  --shard-size "${PRI_V2_SHARD_SIZE:-50}" \
  --workers "${PRI_V2_WORKERS:-3}" \
...
rc=$?
echo "----- finished PR-INJECTOR v2 B500 construction ${RUN_TAG}: rc=${rc} -----"
date
exit "${rc}"
