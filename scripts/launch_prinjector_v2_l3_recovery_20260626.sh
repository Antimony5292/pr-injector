...
RUN_TAG="${PRI_V2_L3_RECOVERY_RUN_TAG:-20260626}"
OUT_DIR="experiments/rq2_500/v2_l3_recovery_${RUN_TAG}"
CANDIDATES="experiments/rq2_500/v2_l3_recovery_pool_20260626/recovery_candidates.jsonl"
LOG_DIR="experiments/rq2_500/logs"
LOG_FILE="${LOG_DIR}/prinjector_v2_l3_recovery_${RUN_TAG}.log"
...
echo
echo "----- start PR-INJECTOR v2 L3 recovery ${RUN_TAG} -----"
date
echo "root=${ROOT}"
...
"${PY}" scripts/run_rq2_b500_fidelity_new_l1l2_shards_20260613.py \
  ...
  --ignore-processed \
  --limit "${PRI_V2_L3_RECOVERY_LIMIT:-47}" \
  --shard-size "${PRI_V2_L3_RECOVERY_SHARD_SIZE:-16}" \
  --workers "${PRI_V2_L3_RECOVERY_WORKERS:-1}" \
...
rc=$?
echo "----- finished PR-INJECTOR v2 L3 recovery ${RUN_TAG}: rc=${rc} -----"
date
exit "${rc}"
