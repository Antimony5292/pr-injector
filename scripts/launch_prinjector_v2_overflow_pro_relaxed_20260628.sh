...
RUN_TAG="${PRI_V2_OVERFLOW_PRO_RUN_TAG:-20260628}"
OUT_DIR="experiments/rq2_500/v2_overflow_pro_relaxed_${RUN_TAG}"
CANDIDATES="experiments/rq2_500/v2_overflow_pro_relaxed_pool_20260628/v2_main_construction_candidates.jsonl"
LOG_DIR="experiments/rq2_500/logs"
LOG_FILE="${LOG_DIR}/prinjector_v2_overflow_pro_relaxed_${RUN_TAG}.log"
...
echo
echo "----- start PR-INJECTOR v2 overflow Pro relaxed ${RUN_TAG} -----"
date
echo "root=${ROOT}"
...
"${PY}" scripts/run_rq2_b500_fidelity_new_l1l2_shards_20260613.py \
  ...
  --ignore-processed \
  --limit "${PRI_V2_OVERFLOW_PRO_LIMIT:-9}" \
  --shard-size "${PRI_V2_OVERFLOW_PRO_SHARD_SIZE:-9}" \
  --workers "${PRI_V2_OVERFLOW_PRO_WORKERS:-1}" \
...
rc=$?
echo "----- finished PR-INJECTOR v2 overflow Pro relaxed ${RUN_TAG}: rc=${rc} -----"
date
exit "${rc}"
