...
RUN_TAG="${PRI_V2_PRO_VERIFIED_BALANCED_RUN_TAG:-20260701}"
OUT_DIR="experiments/rq2_500/v2_pro_verified_balanced_${RUN_TAG}"
CANDIDATES="experiments/rq2_500/v2_pro_verified_balanced_pool_20260701_recycle/v2_main_construction_candidates.jsonl"
LOG_DIR="experiments/rq2_500/logs"
LOG_FILE="${LOG_DIR}/prinjector_v2_pro_verified_balanced_${RUN_TAG}.log"
...
echo
echo "----- start PR-INJECTOR v2 Pro/Verified balanced ${RUN_TAG} -----"
date
echo "root=${ROOT}"
...
export PRI_L3_APPLY_ATTEMPTS="${PRI_L3_APPLY_ATTEMPTS:-4}"
...
"${PY}" scripts/run_rq2_b500_fidelity_new_l1l2_shards_20260613.py \
  ...
  --ignore-processed \
  --limit "${PRI_V2_PRO_VERIFIED_BALANCED_LIMIT:-305}" \
  --shard-size "${PRI_V2_PRO_VERIFIED_BALANCED_SHARD_SIZE:-24}" \
  --workers "${PRI_V2_PRO_VERIFIED_BALANCED_WORKERS:-3}" \
...
rc=$?
echo "----- finished PR-INJECTOR v2 Pro/Verified balanced ${RUN_TAG}: rc=${rc} -----"
date
exit "${rc}"
