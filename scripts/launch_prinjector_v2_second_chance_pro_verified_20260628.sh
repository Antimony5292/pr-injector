...
RUN_TAG="${PRI_V2_SECOND_CHANCE_RUN_TAG:-20260628}"
OUT_DIR="experiments/rq2_500/v2_second_chance_pro_verified_${RUN_TAG}"
CANDIDATES="experiments/rq2_500/v2_second_chance_pro_verified_pool_20260628/second_chance_candidates.jsonl"
LOG_DIR="experiments/rq2_500/logs"
LOG_FILE="${LOG_DIR}/prinjector_v2_second_chance_pro_verified_${RUN_TAG}.log"
...
echo
echo "----- start PR-INJECTOR v2 second-chance Pro/Verified ${RUN_TAG} -----"
date
echo "root=${ROOT}"
...
"${PY}" scripts/run_rq2_b500_fidelity_new_l1l2_shards_20260613.py \
  ...
  --ignore-processed \
  --limit "${PRI_V2_SECOND_CHANCE_LIMIT:-163}" \
  --shard-size "${PRI_V2_SECOND_CHANCE_SHARD_SIZE:-32}" \
  --workers "${PRI_V2_SECOND_CHANCE_WORKERS:-3}" \
...
rc=$?
echo "----- finished PR-INJECTOR v2 second-chance Pro/Verified ${RUN_TAG}: rc=${rc} -----"
date
exit "${rc}"
