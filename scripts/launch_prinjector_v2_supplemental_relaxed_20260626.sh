...
RUN_TAG="${PRI_V2_SUPPLEMENTAL_RUN_TAG:-20260626}"
OUT_DIR="experiments/rq2_500/v2_supplemental_relaxed_${RUN_TAG}"
CANDIDATES="experiments/rq2_500/v2_supplemental_pool_relaxed_20260626/v2_main_construction_candidates.jsonl"
LOG_DIR="experiments/rq2_500/logs"
LOG_FILE="${LOG_DIR}/prinjector_v2_supplemental_relaxed_${RUN_TAG}.log"
...
echo
echo "----- start PR-INJECTOR v2 supplemental relaxed ${RUN_TAG} -----"
date
echo "root=${ROOT}"
...
"${PY}" scripts/run_rq2_b500_fidelity_new_l1l2_shards_20260613.py \
  ...
  --ignore-processed \
  --limit "${PRI_V2_SUPPLEMENTAL_LIMIT:-317}" \
  --shard-size "${PRI_V2_SUPPLEMENTAL_SHARD_SIZE:-40}" \
  --workers "${PRI_V2_SUPPLEMENTAL_WORKERS:-1}" \
...
rc=$?
echo "----- finished PR-INJECTOR v2 supplemental relaxed ${RUN_TAG}: rc=${rc} -----"
date
exit "${rc}"
