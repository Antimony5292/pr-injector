...
RUN_TAG="${REPOLAUNCH_RUN_TAG:-20260628_smoke}"
...
echo
echo "----- start RepoLaunch x PR-INJECTOR smoke ${RUN_TAG} -----"
date
echo "root=${ROOT}"
...
"${PY}" scripts/build_repolaunch_smoke_inputs.py \
  ...
  --limit "${REPOLAUNCH_SMOKE_LIMIT:-3}" \
  --workers "${REPOLAUNCH_WORKERS:-1}" \
  --model "${REPOLAUNCH_MODEL:-bedrock/global.anthropic.claude-sonnet-4-6}" \
  --max-steps-setup "${REPOLAUNCH_MAX_STEPS_SETUP:-12}" \
  --max-steps-verify "${REPOLAUNCH_MAX_STEPS_VERIFY:-8}" \
  --cmd-timeout "${REPOLAUNCH_CMD_TIMEOUT:-10}" \
...
echo "----- finished RepoLaunch x PR-INJECTOR smoke ${RUN_TAG} -----"
date
