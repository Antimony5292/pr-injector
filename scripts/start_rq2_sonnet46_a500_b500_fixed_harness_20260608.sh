...
if [[ -s "${A_PID_FILE}" ]] && kill -0 "$(cat "${A_PID_FILE}")" 2>/dev/null; then
  ...
  nohup bash "/scripts/launch_rq2_sonnet46_a500_fixed_harness_official_20260608.sh" >"${A_START_LOG}" 2>&1 &
  echo "$!" >"${A_PID_FILE}"
...
if [[ -s "${B_PID_FILE}" ]] && kill -0 "$(cat "${B_PID_FILE}")" 2>/dev/null; then
  ...
  nohup bash "/scripts/launch_rq2_sonnet46_b500_fixed_harness_20260608.sh" >"${B_START_LOG}" 2>&1 &
  echo "$!" >"${B_PID_FILE}"
